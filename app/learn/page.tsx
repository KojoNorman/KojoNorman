...
import { ArrowLeft, BookOpen, Calculator, Beaker, Monitor, X, Book, Loader2, Feather } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#FDF6F0] font-sans p-6 md:p-10">
         ...
         {activeTab === 'textbooks' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {filteredLessons.length === 0 ? <div className="col-span-full text-center py-20 text-orange-300 font-bold">No textbooks found.</div> : filteredLessons.map((lesson) => (
                  <div key={lesson.id} onClick={() => setReadingItem(lesson)} className="group bg-white rounded-4xl p-6 shadow-xl shadow-orange-100 border-2 border-transparent hover:border-orange-200 transition-all cursor-pointer hover:-translate-y-1 relative overflow-hidden">
      ...
      {readingItem && (
         <div className="fixed inset-0 z-50 bg-white/95 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
            <div className="max-w-4xl mx-auto bg-white shadow-2xl min-h-screen relative">
               ...
               <div className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-100 p-4 flex items-center justify-between z-10">
                  <div>
                     <h2 className="font-black text-slate-800 text-lg truncate max-w-50 md:max-w-md">{readingItem.title}</h2>
